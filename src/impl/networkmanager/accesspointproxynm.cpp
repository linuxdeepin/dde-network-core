...
{
    // 用原始字节做 locale 感知解码(对齐 nm_utils_ssid_to_utf8),避免依赖上游 qt6 的 fromUtf8
    const QByteArray raw = rawSsid();
    if (!raw.isEmpty())
        return ssidToUtf8(raw);
    return m_network->ssid();
}

QByteArray AccessPointProxyNM::rawSsid() const
{
    if (m_network.isNull() || m_network->referenceAccessPoint().isNull())
        return QByteArray();
...
bool AccessPointProxyNM::isWlan6() const
{
    ...
    if (capability.testFlag(NetworkManager::AccessPoint::Capability(0x10))) // NetworkManager::AccessPoint::Capability::He
        return true;
...
void AccessPointProxyNM::initState()
{
    ...
    if (!ssidBytesMatch(wirelessSetting->ssid(), rawSsid(), ssid()))
        return;
...
void AccessPointProxyNM::updateHiddenInfo()
{
    ...
    NetworkManager::Connection::List::iterator itConnection = std::find_if(connections.begin(), connections.end(), [ this ](NetworkManager::Connection::Ptr connection) {
        ...
        if (wirelessSetting.isNull())
            return false;
        return ssidBytesMatch(wirelessSetting->ssid(), rawSsid(), ssid());
    });
    // 如果没有找到连接，就是非隐藏网络
