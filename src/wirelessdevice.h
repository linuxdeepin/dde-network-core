...
public:
    ...
    AccessPoints(AccessPointProxy *proxy, QObject *parent = nullptr);
    ~AccessPoints();
...
class WirelessConnection : public ControllItems
{
    ...
    friend class WirelessDeviceManagerRealize;

public:
    WirelessConnection();
    AccessPoints *accessPoints() const;  // 返回当前对应的wlan的指针
    ~WirelessConnection() override;
