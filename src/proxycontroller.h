...
public:
    ...
    inline ProxyMethod proxyMethod() const { return m_proxyMethod; }        // 返回代理模式
    ...
    void setProxy(const SysProxyType &type,
                  const QString &addr,
                  const QString &port);  // 设置代理信息，type是一个枚举类型，一般是manual模式下设置
    void setProxyAuth(const SysProxyType &type,
                      const QString &userName,
                      const QString &password,
                      bool enable);                                         // 设置代理验证信息，type是一个枚举类型，一般是manual模式下设置
    SysProxyConfig proxy(const SysProxyType &type) const;                   // 获取对应类型下的系统代理数据
    bool systemProxyExist() const { return m_systemProxyExist; }

    /* 应用代理 */
    virtual bool appProxyEnabled() const;
    virtual void setAppProxyEnabled(bool) {}
    virtual void setAppProxy(const AppProxyConfig &) {}                     // 设置应用代理数据
...
Q_SIGNALS:
    ...
    void systemProxyExistChanged(bool isExit);                              // 系统代理配置存在状态
    /* 应用代理 */
    void appEnableChanged(bool enabled);                                    // 应用代理是否启用状态发生变化
...
protected:
    explicit ProxyController(QObject *parent = Q_NULLPTR);
    ~ProxyController();
