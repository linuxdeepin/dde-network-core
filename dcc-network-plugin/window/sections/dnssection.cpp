...
DNSSection::DNSSection(ConnectionSettings::Ptr connSettings, bool isSupportIPV6, QFrame *parent)
    : AbstractSection(parent)
    , m_headItem(new SettingsHead(this))
    , m_connSettings(connSettings)
    , m_ipv4Setting(m_connSettings->setting(Setting::Ipv4).staticCast<Ipv4Setting>())
    , m_ipv6Setting(m_connSettings->setting(Setting::Ipv6).staticCast<Ipv6Setting>())
...
{
    Ipv4Setting::Ptr ipv4Setting = m_ipv4Setting.staticCast<Ipv4Setting>();
    Ipv6Setting::Ptr ipv6Setting = m_ipv6Setting.staticCast<Ipv6Setting>();
...
{
    QList<ButtonEditWidget *>::iterator iter = m_itemsList.begin();
