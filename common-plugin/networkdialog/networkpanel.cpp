...
{
public:
    explicit ScrollArea(NetworkPanel *panel)
        : QScrollArea(nullptr)
        , m_panel(panel)
    {
    }
    ...
    for (WirelessDevice *device : wirelessDevices) {
        ...
        if (device->isEnabled() && !device->hotspotEnabled()) {
            ...
            if (!m_airplaneMode->enabled()) {
                ...
                if (!apCtrl) {
                    apCtrl = new WirelessItem(m_netListView->viewport(), device, nullptr, this);
                    connect(apCtrl, &WirelessItem::sizeChanged, this, &NetworkPanel::refreshItems);
                }
                apCtrl->updateView();
...
QString NetworkPanel::ssidWaitingForPassword()
{
    return m_reconnectSsid;
}

bool NetworkPanel::closeOnClear() const
{
    return m_closeOnClear;
}

void NetworkPanel::setCloseOnClear(bool closeOnClear)
{
...
void NetworkPanel::clear()
{
    ...
    if (oldSelectItem) {
        ...
        selectItem->expandWidget(WirelessItem::Hide);
    }
    m_applet->verticalScrollBar()->setValue(0);
}
...
void NetworkDelegate::drawSwitchButton(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    DPalette palette = option.palette;
    painter->setBrush(palette.color(DPalette::ColorRole::Button));
